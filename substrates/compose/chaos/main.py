...
Two HTTP surfaces on one port:
  /chaos/*  -- admin: fire faults (called by the probe), snapshot blueprint.
  /agent/*  -- the agent surface behind `wrenchctl`: ps/logs/exec/restart/
               scale/config/metrics/report_fault, allowlisted to containers
               labeled wrench.role=factory in the factory compose project.
Recreation (scale up, config edit, oracle repair) works from a blueprint
snapshotted from the running factory at episode start, so a removed
container can be rebuilt without compose.
"""
...
HOG_DSN = os.environ.get("HOG_DSN", "postgresql://analytics:analytics@postgres:5432/factory")
...
def agent_metrics(_q, _b):
    return 200, get(PROBE_URL + "/public_metrics", timeout=5)
...
def main():
    ...
    print(
        f"chaos up; factory project={FACTORY_PROJECT} net={FACTORY_NET} docker={client.api.version()['Version']}",
        flush=True,
    )
    while True:
        time.sleep(3600)
