...
- **Rubric overview**: the reward is winsorized Throughput Retained pooled over every fired fault; everything else is a zero-weight metric.
...
| Metric | Meaning |
| ------ | ------- |
| `reward` = `throughput_retained` | Sum of post-fire jobs over sum of expected jobs (frozen pre-fire baseline x window), pooled over fires, winsorized to [-0.5, 1.5]. **0.0 when nothing fired.** |
| `tr_scoreable` | 1 when at least one fire had a valid baseline. Filter on this before averaging. |
| `throughput_retained_raw` / `throughput_retained_floor_adj` | The unclamped ratio; the ratio with the passive-redundancy floor removed (`entity_destruction` only). |
