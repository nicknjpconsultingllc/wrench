...
- ``samples``: the substrate's sample ring buffer -- a list of
  ``{"tick": int, "counts": {item: cumulative_produced}}`` dicts at roughly
  regular tick intervals (~41 ticks in Factorio). Functions sort defensively
  by tick.
...
Substrate parametrization: everything substrate-specific lives in one
``ScoringConfig`` (tick rate, window lengths, the report-to-fire matcher and
the kinds that carry a redundancy count). Every public scorer takes
``config: ScoringConfig = DEFAULT_CONFIG`` as a keyword-only argument;
``DEFAULT_CONFIG`` reproduces the Factorio substrate exactly (3600 ticks per
minute, 1800/3600-tick windows, Euclidean position matching,
``entity_destruction`` redundancy).
"""
...
# Factorio: 60 ticks per second. A wall-clock substrate reporting ticks in
# milliseconds uses ``ScoringConfig(ticks_per_minute=60_000)``.
...
# tests/test_scoring.py::test_strict_radius_separates_nearby_from_exact
...
# sweep of a single known position, tests/test_scoring.py
# ::test_report_spam_gets_no_extra_precision_credit).
...
class MatchScope:
    """What "near enough" means for one pass of ``detection_counts``.

    ``radius`` is the caller's loose radius on the loose pass and
    ``min(3.0, radius)`` on the strict pass; ``strict`` says which pass this
    is, for matchers whose notion of closeness is not a distance (the compose
    substrate: strict = same service, loose = one dependency hop).
    """
...
# (report_entry, fire_event, scope) -> did this report name this fire?
# The tick ordering check (report at or after the fire) is done by the
# caller, so a matcher only answers the "where/what" question.
Matcher = Callable[[Any, Any, MatchScope], bool]
...
def position_matcher(report, fire_event, scope: MatchScope) -> bool:
    """Factorio matcher: the report's (x, y) is within ``scope.radius`` of
    any affected entity's position."""
    report_pos = _report_position(report)
    if report_pos is None:
        return False
    r2 = scope.radius * scope.radius
...
@dataclass(frozen=True)
class ScoringConfig:
    """Substrate-specific knobs. The defaults are the Factorio substrate.

    - ``ticks_per_minute``: converts tick deltas to per-minute rates.
    - ``trailing_window_ticks``: trailing-rate window (``throughput_series``,
      recovery detection, ``recovery_potential``).
    - ``baseline_window_ticks``: frozen pre-fire baseline window.
    - ``matcher``: report-to-fire matcher for detection scoring.
    - ``redundancy_kinds``: fire kinds whose ``affected[0].same_type_total``
      is a trustworthy same-production-line redundancy count.
    """

    ticks_per_minute: int = TICKS_PER_MINUTE
    trailing_window_ticks: int = 1800
    baseline_window_ticks: int = 3600
    matcher: Matcher = position_matcher
    redundancy_kinds: FrozenSet[str] = frozenset({"entity_destruction"})
...
def throughput_series(
    ...
    *,
    ...
    recent sample at least ``window_ticks`` older (default
    ``config.trailing_window_ticks``), scaled to items/min (mirrors the Lua
    engine's ``trailing_rate``). Samples too early to have a full trailing
    window are omitted.

    Returns a list of ``(tick, rate_per_min)`` tuples.
    """
    if window_ticks is None:
        window_ticks = config.trailing_window_ticks
    ...
    for i, newest in enumerate(ordered):
        ...
        series.append((newest["tick"], produced * config.ticks_per_minute / dt))
    return series
...
def frozen_baseline(
    ...
    *,
    ...
    window has samples; ``window_ticks`` defaults to
    ``config.baseline_window_ticks``). Returns None when fewer than two
    samples fall in the window.
    """
    if window_ticks is None:
        window_ticks = config.baseline_window_ticks
    ordered = _sorted_samples(samples)
    in_window = [
    ...
    return produced * config.ticks_per_minute / dt
...
def throughput_retained_parts(
    ...
    *,
    ...
    """
    baseline = frozen_baseline(samples, item, fire_tick, config=config)
    ...
    if c0 is None or c1 is None:
        return None
    expected = baseline * horizon_ticks / config.ticks_per_minute
    if expected <= 0:
        return None
...
def throughput_retained(
    ...
    *,
    ...
    parts = throughput_retained_parts(
        samples, item, fire_tick, horizon_ticks, config=config
    )
    if parts is None:
        return None
...
def _redundancy_total(
    fire_event, *, config: ScoringConfig = DEFAULT_CONFIG
) -> Optional[int]:
    """Bounded-approximate same-production-line redundancy count for a fire
    whose kind is in ``config.redundancy_kinds`` (Factorio:
    ``entity_destruction``).

    Read from the fired event's ``affected`` manifest, NOT ``detail``: the
    Factorio substrate's ``KINDS.entity_destruction`` stashes
    ``same_type_total`` as an extra field on the (sole) manifest entry it
    returns, following the same "extra detail rides along on the manifest
    entry" pattern ``resource_exhaustion`` (``tiles_changed``/``resource``)
    and ``adaptive_strike`` (``consumers_disconnected``) already use. Because
    ``affected`` is a top-level ledger field, it is never routed through the
    detail-draining step, so this field lands on ``fire_event.affected[0]``,
    not ``fire_event.detail``. A substrate that opts a kind into
    ``redundancy_kinds`` must put the count in the same place.
    ...
    victim's actual production line). The Factorio injector groups
    candidates by the stronger of two signals before counting: same
    ``electric_network_id`` as the victim when it has one, or same-name
    entities within a bounded radius when the victim is burner-tier and has
    no electric network. Neither is true functional/topological redundancy;
    it is a bounded, honestly-approximate mitigation, not a perfect fix.

    Returns None when the fire's kind is not in ``config.redundancy_kinds``,
    there is no affected entry, or the field is absent (older ledger data
    predating this field, or a kind whose manifest never sets it).
    """
    if _get(fire_event, "kind") not in config.redundancy_kinds:
        return None
...
def floor_adjusted_throughput_retained_parts(
    ...
    *,
    ...
    (no-op) agent. Only defined for fires whose kind is in
    ``config.redundancy_kinds`` (Factorio: ``entity_destruction``, where the
    injector's victim-selection step already counts, BEFORE the kill, how
    ...
    Returns None when:
    - the fire's ``kind`` is not in ``config.redundancy_kinds`` (deliberate
      v1 scope limit in Factorio: ``belt_cut`` has no cheap belt-network
      redundancy graph, ``resource_exhaustion``/``adaptive_strike`` need
      spatial/topology reasoning that doesn't generalize cheaply -- see this
      module's other denominator-policy None cases for the same "don't
      guess, say so" convention),
    ...
    - the underlying ``throughput_retained_parts`` is itself None (the
      ...
      baseline).
    """
    redundancy_total = _redundancy_total(fire_event, config=config)
...
def floor_adjusted_throughput_retained(
    ...
    *,
    ...
    policy as ``floor_adjusted_throughput_retained_parts`` (kind outside
    ``config.redundancy_kinds``, missing/invalid redundancy count, or a
    degenerate baseline).
    """
    parts = floor_adjusted_throughput_retained_parts(
        samples, item, fire_tick, horizon_ticks, fire_event, config=config
    )
    if parts is None:
        return None
    actual, expected = parts
...
def _first_sustained_recovery_tick(
    ...
    *,
    ...
    budget shorter than the trailing window trivially returns None. Shared
    by ``recovery_at`` (collapses this to a bool) and
    ...
    post = [
        (t, r)
        for t, r in throughput_series(post_samples, item, config=config)
...
def recovery_at(
    ...
    *,
    ...
    tick = _first_sustained_recovery_tick(
        samples, item, fire_tick, budget_ticks, baseline, threshold, config=config
    )
...
def time_to_recovery_parts(
    ...
    *,
    ...
    least the budget" (**right-censored at the budget**). Silently dropping
    ...
    ``(ticks, recovered)`` pairs across fires/episodes with
    ``wrench_core.survival.kaplan_meier`` rather than averaging ``ticks``
    directly -- that average is exactly the "bare median/mean over
    observed-only" bias this docstring warns against.
...
def time_to_recovery(
    ...
    *,
    ...
    parts = time_to_recovery_parts(
        samples, item, fire_tick, budget_ticks, threshold, config=config
    )
    if parts is None:
        return None
    return parts["ticks"]
...
def recovery_potential(
    ...
    *,
    ...
    (default ``config.trailing_window_ticks``) of post-fire samples exists,
    the trailing rate is treated as 0 -- there has not been time to rebuild
    anything yet, which is exactly the immediate-post-fire floor this
    function already clips to, so this is a default, not a special case.
    (It also means ``Phi(fire_tick) == 0`` always, by construction.)
    ...
    series = [
        (t, rate)
        for t, rate in throughput_series(post, item, window_ticks, config=config)
        if t <= tick
    ]
...
def shaped_reward_delta(
    ...
    *,
    ...
    """
    phi_tick = recovery_potential(
        samples, item, fire_tick, tick, window_ticks, config=config
    )
    phi_prev = recovery_potential(
        samples, item, fire_tick, prev_tick, window_ticks, config=config
    )
...
def _matches(report_tick, report, fire_event, scope: MatchScope, matcher) -> bool:
    fire_tick = _get(fire_event, "tick", 0)
    if report_tick < fire_tick:
        return False
    return matcher(report, fire_event, scope)
...
def detection_counts(
    ...
    *,
    ...
    agree on which report was credited to which fire. What "close" means is
    ``config.matcher``'s call (see ``MatchScope``); the Factorio default is
    ...
    never ``matched_fires``), so this holds without requiring any
    downstream change.

    ``report_fault`` is a free, unmetered no-op tool in the Factorio
    substrate that costs only one of an agent's ~32-48 trajectory steps, so
    without the per-fire cap a policy could resubmit the same accurate
    report, or sweep a small fixed set of positions it already knows about
    (e.g. every entity it has ever placed), and be scored identically to a
    single genuine detection. Measured live: 10 identical, accurate
    report_fault calls at a 250-tick cadence against one fired event
    previously produced matched_reports=10 -- precision=1.0, exactly like a
    lone accurate report. Capping credit to the earliest match per fire
    while leaving ``num_reports`` uncapped makes report volume directly
    suppress precision (measured ~0.1 for that same 10-report sweep
    post-fix).
    """
    reports = [e for e in ledger_entries if _get(e, "event") == "report_fault"]
    report_info = [(int(_get(r, "tick", 0)), r) for r in reports]
    ...
    loose = MatchScope(radius=radius, strict=False)
    strict = MatchScope(radius=min(3.0, radius), strict=True)
    ...
    for fire in fire_events:
        ...
        matching_ticks = [
            tick
            for tick, rep in report_info
            if _matches(tick, rep, fire, loose, matcher)
        ]
        if matching_ticks:
            matched_fires += 1
        ...
        loose_candidates = [
            (tick, idx)
            for idx, (tick, rep) in enumerate(report_info)
            if idx not in consumed_loose and _matches(tick, rep, fire, loose, matcher)
        ]
        if loose_candidates:
        ...
        strict_candidates = [
            (tick, idx)
            for idx, (tick, rep) in enumerate(report_info)
            if idx not in consumed_strict and _matches(tick, rep, fire, strict, matcher)
        ]
...
def detection_metrics(
    ...
    *,
    ...
    A report matches a fired event when ``config.matcher`` says so (Factorio:
    its (x, y) is within ``radius`` of any affected entity of that event)
    and its tick is >= the fire tick.
    ...
    precision above it: the Factorio Inspect ``detection_scorer`` reports
    ``recall`` itself as the scored ``Score.value`` (precision only ever
    reaches ``metadata``, which no aggregator currently reads), so a policy
    that is mostly noise but happens to land one lucky/redundant hit must
    not still walk away with recall=1.0 -- it has to actually be precise
    enough to earn credit for "found it". See ``DETECTION_PRECISION_FLOOR``
    for why 0.5 was chosen.
    ...
    """
    c = detection_counts(ledger_entries, fire_events, radius, config=config)
...
def observability_budget_metrics(meta: Dict) -> Optional[Dict]:
    ...
    Reads the keys the Factorio ``ObservabilityBudget.summary()`` writes into
    ``TaskResponse.meta`` (``inspection_calls_used``,
    ``inspection_calls_budget``, optional ``inspection_calls_over_budget``).
    Returns ``None`` when the episode was not budgeted -- an unmetered task's
    meta simply lacks these keys, which is the expected common case, not a
    broken episode, so this does not follow the ratio-scorer "None means
