...
sibling ``.meta.json`` describing the episode. Substrates enable it by
setting the ``WRENCH_TRAJECTORY_DIR`` environment variable -- see
``for_env()``.
"""
...
class TrajectoryWriter:
    ...
    def append_step(
        ...
        # Omitted from the record entirely when absent, so callers that never
        # pass it see byte-for-byte identical records to before.
